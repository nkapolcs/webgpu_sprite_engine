...
export class SpriteRenderer {
  ...
  public drawString(font: SpriteFont, text: string, position: vec2, color: Color = this.defaultColor, scale = 1) {
    ...
    for (let j = 0; j < text.length; j++) {
      ...
      const x = position[0] + (char.offset[0] + nextCharX) * scale;
  ...
  public drawSpriteSource(texture: Texture, rect: Rect, sourceRect: Rect, color: Color = this.defaultColor, rotation = 0, rotationAnchor: vec2 | null = null) {
    if (this.currentTexture != texture) {
      this.currentTexture = texture;

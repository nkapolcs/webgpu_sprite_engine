...
export class Content {
  ...
  public static async loadSnowBSpriteFont(device: GPUDevice, xmlPath: string, texturePath: string): Promise<SpriteFont> {
    ...
    xmlDoc.querySelectorAll("char").forEach((char) => {
      ...
      const quad = new Quad(vec2.fromValues(x1, y1), vec2.fromValues(x2, y1), vec2.fromValues(x2, y2), vec2.fromValues(x1, y2));

      font.createChar(id, quad, vec2.fromValues(width, height), advance, vec2.fromValues(xOffset, yOffset));
    });
